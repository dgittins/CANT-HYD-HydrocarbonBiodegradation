#!usr/pyenv/python3
#this script is for VALIDATION - will filter everything above TRUSTED CUTOFF and appends the oxygen use and HC_type (aerobic aromatic etc)
...
#            print(line_new)
            if cols[2] in max_score.keys():
                if float(cols[8]) >= float(max_score[cols[2]][0]): #if the domain score is greater than the trusted cutoff (for validation)
